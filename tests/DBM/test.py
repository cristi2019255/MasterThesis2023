...
def test_dbm(name="DBM"):
    path_img_fast = os.path.join(
        os.getcwd(), "models", name, "boundary_map_fast.npy")
    path_img_real = os.path.join(
        os.getcwd(), "models", name, "boundary_map.npy")

    path_confidence_fast = os.path.join(
        os.getcwd(), "models", name, "boundary_map_confidence_fast.npy")
    path_confidence_real = os.path.join(
        os.getcwd(), "models", name, "boundary_map_confidence.npy")
    ...
    print(
        f"Percentage of errors: {errors / (img1.shape[0] * img1.shape[1]) * 100} %")
...
def test_projection_errors():
    ...
    for ii in range(len(embedding)):
        ...
        i, j = int((x - min_x) / (max_x - min_x) *
                   255), int((y - min_y) / (max_y - min_y) * 255)
        img[i, j] = T
