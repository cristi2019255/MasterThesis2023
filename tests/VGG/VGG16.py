...
def build_custom_model(input_shape=(48, 48, 3), include_top=False, output_shape=10, model_name="MNIST"):
    ...
    # initializing model with weights='imagenet'i.e. we are carring its original weights
    console.log(
        f"Initializing VGG16 model with weights='imagenet', input_tensor = {input_shape}, include_top = {include_top}.\n")
    model_vgg16 = VGG16(weights='imagenet',
                        input_tensor=input_layer, include_top=include_top)
...
def get_convolution_corpus(X_train, model):
    ...
    convolution_corpus = convolution_corpus.reshape(
        convolution_corpus.shape[0], convolution_corpus.shape[3])
    console.log(
        f"Reshaping convolution corpus ... The new shape is: {convolution_corpus.shape}")
