...
def freeze_layers(model, layers_to_freeze=1):
    ...
    assert layers_to_freeze < len(
        model.layers), "Number of layers to freeze is smaller than the number of layers in the model."
...
def predict(X_test, Y_test, model):
    ...
    for i in range(15):
        ...
        plt.title(
            f"Predicted: {output} ({pred[output]*100:.2f}%) \n Actual: {np.argmax(y[i])}")
