...
class AbstractDBM:
    ...
    @track_time_wrapper(logger=time_tracker_console)
    def _get_img_dbm_fast_binary_hybrid_strategy(self, resolution: int, computational_budget=None, interpolation_method: str = "linear"):
        """
        ...
        for i in range(INITIAL_RESOLUTION):
            for j in range(INITIAL_RESOLUTION):
                ...
                priority = get_decode_pixel_priority(img, x, y, window_size, predictions[i, j])
                if priority != -1:
                    items_to_decode.append((window_size, x, y))

        # -------------------------------------
        # start the process of filling the image
        space, space_indices = [], []   
        self.console.log(f"Starting the iterative process of refining windows...")
        for (window_size, i, j) in items_to_decode:
            y1, x1, y2, x2 = int(i - window_size / 2), int(j - window_size / 2), int(i + window_size / 2), int(j + window_size / 2)
            for y in range(y1, y2 + 1):
                for x in range(x1, x2 + 1):
                    space_indices += [(y, x)]
                    space += [(y / resolution, x / resolution)]
        ...
        # fill the new image with the new labels and update the priority queue
        for (x, y), label, conf in zip(space_indices, predicted_labels, predicted_confidence):
            confidence_map.append((x, y, conf))
            img[x, y] = label
