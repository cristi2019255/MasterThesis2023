...
class DBMPlotterGUI:
    ...
    def _initialize_gui_(self):
        ...
        self.classifier_performance_canvas, self.classifier_performance_fig_agg = self._build_canvas_(
            self.classifier_performance_fig, key="-CLASSIFIER PERFORMANCE CANVAS-")
        self.canvas, self.fig_agg, self.canvas_controls = self._build_canvas_(
            self.fig, key="-DBM CANVAS-", controls_key="-CONTROLS CANVAS-")
        ...
        self.draw_dbm_img()
        # --------------------- GUI related ---------------------
        self.updates_logger = LoggerGUI(
            name="Updates logger", output=self.window["-LOGGER-"], update_callback=self.window.refresh)
    ...
    def handle_decoding_strategy_change_event(self, event, values):
        self.fig_agg = draw_figure_to_canvas(
            self.canvas, self.fig, self.canvas_controls)
        self.window.refresh()
    ...
    def _build_annotation_mapper_(self):
        ...
        annImage = AnnotationBbox(image, (0, 0), xybox=(
            50., 50.), xycoords='data', boxcoords="offset points",  pad=0.1,  arrowprops=dict(arrowstyle="->"))
        annLabels = AnnotationBbox(label, (0, 0), xybox=(
            50., 50.), xycoords='data', boxcoords="offset points",  pad=0.3,  arrowprops=dict(arrowstyle="->"))
        ...
        def find_data_point(i, j):
            ...
            point = self.dbm_model.neural_network.decode(
                [(i/self.img.shape[0], j/self.img.shape[1])])[0]
            return point, None
        ...
        def onclick(event):
            ...
            if f"{i} {j}" in self.expert_updates_labels_mapper:
                ...
                self.fig.canvas.draw_idle()
                self.updates_logger.log(
                    f"Removed updates for point: ({j}, {i})")
                return

            # if clicked on a data point that was not updated, then add the update
            self.current_selected_point = self.ax.plot(
                j, i, 'bo', markersize=5)[0]
            ...
            self.key_event_cid = self.fig.canvas.mpl_connect(
                'key_press_event', onkey)
            ...
            if event.key == 'escape' or event.key == 'enter':
                ...
                if event.key == 'escape':
                    ...
                    if self.current_selected_point_assigned_label is not None:
                        self.updates_logger.log(
                            f"Assigned label: {self.current_selected_point_assigned_label} to point: ({x[0]}, {y[0]})")
                        self.expert_updates_labels_mapper[f"{y[0]} {x[0]}"] = (
                            self.current_selected_point_assigned_label, self.current_selected_point)

                self.current_selected_point = None
                self.current_selected_point_assigned_label = None

                self.motion_event_cid = self.fig.canvas.mpl_connect(
                    'motion_notify_event', display_annotation)
                self.click_event_cid = self.fig.canvas.mpl_connect(
        ...
        def onclick_circle_strategy(event):
            ...
            self.release_event_cid = self.fig.canvas.mpl_connect(
                'button_release_event', onrelease_circle_strategy)

        def onrelease_circle_strategy(event):
            if event.inaxes == None:
                return
            self.console.log("Released on: " +
                             str(event.xdata) + ", " + str(event.ydata))
            ...
            self.update_labels_circle = Circle(
                (cx, cy), r, color='black', fill=False)
            self.ax.add_artist(self.update_labels_circle)
            self.key_event_cid = self.fig.canvas.mpl_connect(
                'key_press_event', onkey_circle_strategy)
        ...
        def onkey_circle_strategy(event):
            ...
            if event.key == "enter" and self.current_selected_point_assigned_label is not None:
                self.updates_logger.log(
                    f"Assigned label: {self.current_selected_point_assigned_label} to circle: center ({x}, {y}), radius ({r})")
                positions = find_points_in_circle((x, y), r)
                for pos in positions:
                    point = self.ax.plot(pos[0], pos[1], 'b^')[0]
                    self.expert_updates_labels_mapper[f"{pos[1]} {pos[0]}"] = (
                        self.current_selected_point_assigned_label, point)

            if event.key == "backspace":
                self.updates_logger.log(
                    f"Removing changes in circle: center ({x},{y}), radius ({r})")
                positions = find_points_in_circle((x, y), r)
                for pos in positions:
        ...
        def find_points_in_circle(circle_center, circle_radius):
            (cx, cy) = circle_center
            initial_x, initial_y = int(
                cx - circle_radius), int(cy - circle_radius)
            final_x, final_y = int(cx + circle_radius) + \
                1, int(cy + circle_radius) + 1
    ...
    def compute_classifier_metrics(self):
        ...
        loss, accuracy = self.dbm_model.classifier.evaluate(
            self.X_test, self.Y_test, verbose=0)
        self.console.log(
            f"Classifier Accuracy: {(100 * accuracy):.2f}%  Loss: {loss:.2f}")

        path = os.path.join(
            self.save_folder, CLASSIFIER_PERFORMANCE_HISTORY_FILE)
        ...
        self.window["-CLASSIFIER ACCURACY-"].update(
            f"Classifier Accuracy: {(100 * accuracy):.2f} %  Loss: {loss:.2f}")
    ...
    def pop_classifier_evaluation(self):
        ...
        accuracy, loss = float(last_line.split("Accuracy: ")[1].split("%")[
                               0]), float(last_line.split("Loss: ")[1])
        self.window["-CLASSIFIER ACCURACY-"].update(
            f"Classifier Accuracy: {(accuracy):.2f} %  Loss: {loss:.2f}")
        self.update_classifier_performance_canvas()

    def handle_compute_inverse_projection_errors_event(self, event, values):
        self.window['-COMPUTE INVERSE PROJECTION ERRORS-'].update(
            visible=False, disabled=True)
        self.updates_logger.log(
            "Computing inverse projection errors, please wait...")
        possible_path = os.path.join(
            self.save_folder, "inverse_projection_errors.npy")
        ...
        if os.path.exists(possible_path):
            ...
            self.inverse_projection_errors = self.dbm_model.generate_inverse_projection_errors(
                save_folder=self.save_folder, resolution=len(self.img))
    ...
    def _set_loading_proj_errs_state_(self):
        self.window['-COMPUTE PROJECTION ERRORS INTERPOLATION-'].update(
            visible=False, disabled=True)
        self.window['-COMPUTE PROJECTION ERRORS INVERSE PROJECTION-'].update(
            visible=False, disabled=True)
    ...
    def handle_compute_projection_errors_event(self, event, values):
        ...
        if event == "-COMPUTE PROJECTION ERRORS INTERPOLATION-":
            possible_path = os.path.join(
                self.save_folder, "projection_errors_interpolated.npy")
            # try to get projection errors from cache first
            if os.path.exists(possible_path):
                self.projection_errors = np.load(possible_path)
            else:
                self.projection_errors = self.dbm_model.generate_projection_errors(
                    use_interpolation=True, save_folder=self.save_folder)
        elif event == "-COMPUTE PROJECTION ERRORS INVERSE PROJECTION-":
            possible_path = os.path.join(
                self.save_folder, "projection_errors_inv_proj.npy")
            # try to get projection errors from cache first
            if os.path.exists(possible_path):
                self.projection_errors = np.load(possible_path)
            else:
                self.projection_errors = self.dbm_model.generate_projection_errors(
                    use_interpolation=False, save_folder=self.save_folder)
    ...
    def handle_checkbox_change_event(self, event, values):
        ...
        if show_classifier_predictions:
            colors = [COLORS_MAPPER[label]
                      for label in self.encoded_train[:, 2]]
            self.axes_classifier_scatter = self.ax.scatter(
                self.encoded_train[:, 1], self.encoded_train[:, 0], s=10, c=colors)
        ...
        if show_labels_changes and len(positions_x) > 0 and len(positions_y) > 0 and len(alphas) > 0:
            self.ax_labels_changes = self.ax.scatter(
                positions_x, positions_y, s=10, c='green', marker='^', alpha=alphas)
    ...
    def handle_apply_changes_event(self, event, values):
        ...
        if num_changes < 10:
            self.updates_logger.log(
                "Less than 10 changes to apply, please apply more changes")
            return
        ...
        self.updates_logger.log(
            "Applying changes... This might take a couple of seconds, after this the window will be closed")

        save_folder = os.path.join(self.save_folder, CLASSIFIER_REFIT_FOLDER)

        # store the old model so we can restore it when needed
        self.dbm_model.save_classifier(save_folder=os.path.join(
            self.save_folder, CLASSIFIER_STACKED_FOLDER))
    ...
    def transform_changes(self):
        ...
        alphas = [alpha - ALPHA_DECAY_ON_UPDATE if alpha >
                  ALPHA_DECAY_ON_UPDATE else ALPHA_DECAY_ON_UPDATE for alpha in alphas]
    ...
    def update_classifier_performance_canvas(self):
        ...
        self.classifier_performance_fig_agg = draw_figure_to_canvas(
            self.classifier_performance_canvas, self.classifier_performance_fig)
        self.window.refresh()
