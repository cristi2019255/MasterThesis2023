...
class GUI:
    ...
    def handle_upload_train_data_event(self, event, values):
        try:
            ...
            self.X_train, self.Y_train = import_csv_dataset(
                filename, limit=int(0.7*SAMPLES_LIMIT))
            ...
            self.window["-TRAIN DATA FILE-"].update(
                "Training data file: " + filename)
            self.window["-TRAIN DATA SHAPE-"].update(
    ...
    def handle_upload_test_data_event(self, event, values):
        try:
            ...
            self.X_test, self.Y_test = import_csv_dataset(
                filename, limit=int(0.3*SAMPLES_LIMIT))
            if self.classifier is not None and self.X_train is not None and self.Y_train is not None:
                self.switch_visibility(["-DBM BTN-"], True)

            self.window["-TEST DATA FILE-"].update(
                "Testing data file: " + filename)
            self.window["-TEST DATA SHAPE-"].update(
                f"Testing data shape: X {self.X_test.shape} Y {self.Y_test.shape}")
        except Exception as e:
            self.dbm_logger.error("Error while loading data file" + str(e))
    ...
    def _post_uploading_processing_(self):
        ...
        self.window["-TRAIN DATA FILE-"].update(
            f"Training data: {self.dataset_name}")
        self.window["-TRAIN DATA SHAPE-"].update(
            f"Training data shape: X {self.X_train.shape} Y {self.Y_train.shape}")

        self.window["-TEST DATA FILE-"].update(
            f"Testing data: {self.dataset_name}")
        self.window["-TEST DATA SHAPE-"].update(
            f"Testing data shape: X {self.X_test.shape} Y {self.Y_test.shape}")
    ...
    def handle_dbm_technique_event(self, event, values):
        ...
        if dbm_technique == "nnInv":
            self.switch_visibility(
                ["-PROJECTION TECHNIQUE TEXT-", "-PROJECTION TECHNIQUE-"], True)
        else:
            self.switch_visibility(
                ["-PROJECTION TECHNIQUE TEXT-", "-PROJECTION TECHNIQUE-"], False)
    ...
    def handle_get_decision_boundary_mapping_event(self, event, values):
        if self.classifier is None:
            self.dbm_logger.error(
                "No classifier provided, impossible to generate the DBM...")
            return

        if self.X_train is None or self.Y_train is None or self.X_test is None or self.Y_test is None:
            self.dbm_logger.error(
                "Data is incomplete impossible to generate the DBM...")
            return
        ...
        dbm = DBM_TECHNIQUES[values["-DBM TECHNIQUE-"]
                             ](classifier=self.classifier, logger=self.dbm_logger)
        ...
        img.thumbnail((WINDOW_IMAGE_RESOLUTION,
                      WINDOW_IMAGE_RESOLUTION), Image.ANTIALIAS)
